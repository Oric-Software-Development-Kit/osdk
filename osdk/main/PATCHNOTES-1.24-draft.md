...
### New `-Os` optimize-for-size flag
- **Found:** real projects want an explicit "smallest code" switch (Encounter is nearly out of RAM). The compiler only accepted `-O0..-O3` and errored ("Unknown option -Os") on anything else.
- **Fix (gen.c):** `-Os` (and `-OS`) now select optimize-for-size. On the 6502 nearly every optimization is Pareto (smaller *and* faster), and `-O3` is the smallest level (its folds are all size wins), so `-Os` maps to `-O3`-level codegen today — byte-identical output. It also sets an internal size-axis flag for the future transforms that would *trade* size for speed (e.g. unrolling shifts by 4-7): those will be enabled only under a speed axis, and suppressed under `-Os`. First half of the size-vs-speed axis (global flag; the per-function `#pragma optimize(size|speed)` override and the divergent transforms are still to come).
- **Result:** projects can build with `SET OSDKCOMP=-Os`. Encounter's C-heavy modules drop ~11% vs the released compiler.
...
### Peephole: keep `.csource`/`.ctype` debug directives whole (don't split on their `:`)
- **Found:** enabling the peephole (`OSDKMACRO=-O`) on a `-g1` build broke assembly with "Syntax error" / "Label defined" — surfaced on Encounter's SplashProgram. The `-g1` compiler emits debug directives whose payload uses `:` as data: `.csource "C:/path/file.c" 12` (drive-letter/path colon) and `.ctype struct Entity 20 name:char[8]:0:8 kind:EntityKind:8:1 …` (field-separator colons). The macrosplitter tokenizes statements by splitting each line on `:`, so it shredded these directives into a truncated fragment plus garbage "labels" that XA rejected. Never exercisable before: debug-support had no peephole, our branch had no debug info — the merge created the first `-g1`+peephole pairing.
- **Fix (macrosplitter TokenizeLine):** a line beginning `.csource`/`.ctype` is kept whole as one directive token (never split), alongside the existing whole-line comment handling; plus a general quote-aware `:` scan so a `:` inside a `"…"` string is never a statement separator. `.csource` and `.ctype` are the only two debug directives the compiler emits (gen.c), so this covers them.
- **Result:** `-g1`+peephole builds cleanly. Confirmed end-to-end on the debug_type_zoo sample (typedef enums/structs) and on Encounter (`-O3 -g1` + peephole, all modules). Full known-answer suite green at -O1/-O2/-O3, byte-identical on non-debug code (the change only affects lines containing `.csource`/`.ctype`/quotes).
