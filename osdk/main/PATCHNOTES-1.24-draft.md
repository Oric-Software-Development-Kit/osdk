...
- **Scope this phase:** the byte *operation* is emitted (the bulk of the saving); the now-redundant zero-extend (`CZBW`) of the operands is still emitted for safety and will be elided in a follow-up (it can be folded together with the byte load at -O3, so a naive skip is unsound — it briefly regressed aes256 -O2 during development and was reverted to the always-correct form). `<< , unary ~/-, ++/--`, comparisons, `>>` and `*` are later phases.
- **Result:** new `t_char8` known-value test (12 checks incl. wraparound and chained ops) passes at -O1/-O2/-O3; full suite green (no regressions). char-heavy code shrinks (e.g. the test's own body -17% at -O2); aes256 unaffected in correctness.
