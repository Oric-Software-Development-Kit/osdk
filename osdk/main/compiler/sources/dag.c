...
static int haskid(Node p, Node t) {
	if (t == 0)
		return 0;
	else if (p == t)
		return 1;
...
Node node(int op, Node l, Node r, Symbol sym) {
...
static Node undag(Node nodelist) {
	Node p, pred;
	struct node head;
...
static Node undag1(Node p, Node root) {
	Node e;
