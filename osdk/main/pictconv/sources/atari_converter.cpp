...
{
  SetFormat(FORMAT_SINGLE_PALETTE);
...
bool AtariPictureConverter::SetFormat(int format)		                
{ 
...
void AtariPictureConverter::set_buffer_size(int width,int height)
{
  ...
  m_buffer_size         =m_buffer_height*((width+15)/16)*2*m_buffer_bitplans;	// *wordsize*bitplanscount;

  int pictureCount=1;
  if (m_swapping==SWAPPING_ENABLED)
  {
    pictureCount=2;
  }
  delete[] m_buffer;
  m_buffer=new unsigned char[m_buffer_size*pictureCount];
}
...
bool AtariPictureConverter::Convert(const ImageContainer& sourcePicture)
{
  ...
  case FORMAT_MONOCHROME:
    convertedPicture.ConvertToGrayScale(256);
    {
      /*
      AtariClut clut;
      clut.SetColor(RgbColor(0,0,0),0);
      clut.SetColor(RgbColor(128,128,128),1);
      clut.SetColor(RgbColor(255,255,255),2);

      convertedPicture.ReduceColorDepth(&clut);
      */
    }
...
void AtariPictureConverter::convert_shifter_monochrome(const ImageContainer& sourcePicture)
{
  // 
  // The Atari ST monochrome format is relatively simple: 640x400 pixels, 80 bytes per scanline
  {
    unsigned char *ptr_hires     =GetBufferData(0);
    unsigned int swapOffset = 0;
    if (m_swapping==SWAPPING_ENABLED)
    {
      swapOffset=m_buffer_width/8;
    }
    ...
    for (unsigned int y=0;y<m_buffer_height;y++)
    {
      ...
      while (x<m_buffer_width)
      {
        ...
        if (swapOffset)
        {
          ptr_hires[swapOffset+0]=static_cast<unsigned char>((p0_swap>>8)&255);
          ptr_hires[swapOffset+1]=static_cast<unsigned char>(p0_swap&255);
        }
        *ptr_hires++=static_cast<unsigned char>((p0>>8)&255);
        *ptr_hires++=static_cast<unsigned char>(p0&255);
      }
...
bool AtariPictureConverter::TakeSnapShot(ImageContainer& sourcePicture)
{
  ...
  if (!sourcePicture.Allocate(m_buffer_width,m_buffer_height*pictureCount,32))
  {
    return false;
  }
  ...
  case FORMAT_MONOCHROME:
    {
      for (unsigned int y=0;y<m_buffer_height;y++)
      {
        ...
        {
          unsigned short* pShort=(unsigned short*)m_buffer+pictureCount*m_buffer_width;
          for (unsigned int x=0;x<m_buffer_width;x+=16)
          {
            unsigned short p0=*pShort++;
...
void AtariPictureConverter::SaveToFile(long handle,int output_format)
{
  ...
  for (int picture=0;picture<pictureCount;picture++)
  {
    write(handle,GetBufferData(picture),GetBufferSize());
  }
}
