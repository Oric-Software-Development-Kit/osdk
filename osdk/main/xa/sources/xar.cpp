...
void SymbolData::ExportEquates(FILE *fp, const char* prefix_filter, const char* exclude_file)
{
	// Build exclusion set from symbol file (format: "hexaddr name" per line)
	std::set<std::string> excluded;
	if (exclude_file && exclude_file[0])
	{
		FILE *xf = fopen(exclude_file, "r");
		if (xf)
		{
			char line[1024];
			while (fgets(line, sizeof(line), xf))
			{
				char addr[64], name[512];
				if (sscanf(line, "%63s %511s", addr, name) == 2)
		...
		{
			fprintf(stderr, "Warning: -X file '%s' not found, ignoring\n", exclude_file);
		}
	}

	fprintf(fp, "; Auto-generated by XA - do not edit\n");
	SymbolEntry *ptr_symbol_entry;
	for (int i = 0; i < m_nb_labels; i++)
	{
		ptr_symbol_entry = m_ptr_table_entries + i;
		if (ptr_symbol_entry->m_block_level != 0)                       continue;
		if (ptr_symbol_entry->symbol_status != eSYMBOLSTATUS_VALID)     continue;
		...
		if (prefix_filter && prefix_filter[0]
		 && strncmp(ptr_symbol_entry->ptr_label_name, prefix_filter, strlen(prefix_filter)) != 0)
			continue;
		if (!excluded.empty() && excluded.count(ptr_symbol_entry->ptr_label_name))
			continue;
		fprintf(fp, "%s = $%04x\n", ptr_symbol_entry->ptr_label_name, ptr_symbol_entry->value & 0xffff);
	}
}
